...
class DoorAccessGridDataSource<T> extends EmrQueryGraphQLDataSource<T> {
  DoorAccessGridDataSource({required this.gridData, this.toObjectString});
  ...
  List<T> filterData() {
    ...
    return gridData
        .where(
          (item) => toObjectString!(
            item,
          ).toLowerCase().contains(queryScope.query.toLowerCase()),
        )
        .toList();
  }
}
