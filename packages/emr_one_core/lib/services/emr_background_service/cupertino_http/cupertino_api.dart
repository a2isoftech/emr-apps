...
import 'package:emr_one_core/services/emr_background_service/cupertino_http/utils.dart';
...
class URLSession extends _ObjectHolder<ncb.NSURLSession> {
  ...
  static ncb.NSURLSessionDelegate delegate(
    ...
    void Function(URLSession session, URLSessionTask task, int bytesSent,
            int totalBytesSent, int totalBytesExpectedToSend)?
        onDidSendBodyData,
  }) {
    ...
    if (onDidSendBodyData != null) {
      ncb.NSURLSessionDataDelegate
          .URLSession_task_didSendBodyData_totalBytesSent_totalBytesExpectedToSend_
          .implementAsListener(protoBuilder, (nsSession, nsTask, bytesSent,
      ...
      return URLSession._(
        ncb.NSURLSession.sessionWithConfiguration_delegate_delegateQueue_(
            ...
            delegate(config._isBackground,
                ...
                onDidSendBodyData: onDidSendBodyData),
            queue),
  ...
  /// Create a [URLSessionTask] that uploads data to a server URL.
  URLSessionTask upload(URLRequest request, Uri file) =>
      URLSessionTask._(_nsObject.uploadTaskWithRequest_fromFile_(
          request._nsObject, uriToNSURL(file)));
}

class URLSessionUploadTask extends _ObjectHolder<ncb.NSURLSessionUploadTask> {
  URLSessionUploadTask._(super.c);
}
