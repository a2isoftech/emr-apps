...
import 'package:emr_one_elv_inspection/database_raven/models/vehicle_parts_model.dart';
import 'package:flutter/foundation.dart';
import 'package:hive/hive.dart';

class VehiclePartsDatabase {
  VehiclePartsDatabase(this._vehicleKey) {
    if (!kIsWeb) {
  ...
  Future<void> changeAlloyWheelsCount(int count) async {
    ...
    await _updateVehiclePart(
      ...
      createIfAbsent: () =>
          VehicleParts(vehicleKey: _vehicleKey, alloyWheelCount: count),
    );
  }
  ...
  Future<void> changeSteelWheelsCount(int count) async {
    ...
    await _updateVehiclePart(
      ...
      createIfAbsent: () =>
          VehicleParts(vehicleKey: _vehicleKey, steelWheelCount: count),
    );
  }
  ...
  Future<void> changeChromeWheelsCount(int count) async {
    ...
    await _updateVehiclePart(
      ...
      createIfAbsent: () =>
          VehicleParts(vehicleKey: _vehicleKey, chromeWheelCount: count),
    );
  }
  ...
  Future<void> changeBatteryStatusCount({bool? status}) async {
    ...
    await _updateVehiclePart(
      ...
      createIfAbsent: () =>
          VehicleParts(vehicleKey: _vehicleKey, hasBattery: status),
    );
  }
  ...
  MapEntry<int, VehicleParts>? getVehiclePart() {
    ...
    return _box.values
        .where((part) => part.vehicleKey == _vehicleKey)
  ...
  Future<void> changeVinValidateStatus({required bool status}) async {
    ...
    await _updateVehiclePart(
      ...
      createIfAbsent: () =>
          VehicleParts(vehicleKey: _vehicleKey, isValidVin: status),
    );
  }
}
