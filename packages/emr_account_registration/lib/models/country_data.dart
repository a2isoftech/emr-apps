...
class CountryData {
  CountryData(this.dialingCode, this.shortName, this.fullName, this.images);
  ...
  static List<CountryData>? _countriesData;

  static final _countries = [
    {'name': 'United Kingdom', 'code': 'UK', 'dial_code': '+44'},
  ...
  static CountryData? getCountryData(String countryCode) {
    if (countryCode.isNotEmpty) {
      final cntCode = countryCode == 'GB' ? 'UK' : countryCode;
      final cnt = _countries.firstWhereOrNull((x) => x['code'] == cntCode);
      if (cnt != null) {
        return CountryData(
          cnt['dial_code'] ?? '',
          cnt['code'] ?? '',
          cnt['name'] ?? '',
          'assets/images/flags/${(cnt['code'] ?? '').toLowerCase()}.png',
        );
      }
    }
    return null;
  ...
  static Future<List<CountryData>> populateCountries() async {
    _countriesData = _countries
        .map(
          (e) => CountryData(
            ...
            e['code'] ?? '',
            e['name'] ?? '',
            e['dial_code'] != null && e['dial_code']!.isNotEmpty
  ...
  static List<CountryData> getCounties(BuildContext context) {
    //TODO: add localisations for country data
    return _countriesData ?? [];
  }
...
extension CountryDataExtension on List<CountryData> {
  List<CountryData> insertSelect(String text) {
    if (isNotEmpty && this[0].shortName != text) {
      insert(0, CountryData('', text, text, ''));
    }
    return this;
  }
}
