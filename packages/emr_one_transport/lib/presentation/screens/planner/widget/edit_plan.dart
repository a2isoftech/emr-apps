...
class EditPlan extends StatefulWidget {
  const EditPlan({required this.planId, required this.appConfig, super.key});
...
class _EditPlanState extends State<EditPlan> {
  ...
  void initState() {
    ...
    controller.fetchPlanDetails(widget.planId);
    super.initState();
  }
                ...
                Expanded(
                  child: EmrTabBarVerticalView(
                    ...
                    onSave: () async {
                      ...
                      final response = await controller.updatePlan();
                      if (context.mounted) {
                        if (response.isSuccess) {
                          await SuccessAlertWidget.showSuccessMessage(
                            context,
                            context.l10n.planUpdatedSuccessfully,
                          );
                          if (context.mounted) {
                    ...
                    buttons: [
                      ...
                      TextButton(
                        ...
                        onPressed: () => EmrDialog.modal<void>(
                          ...
                          titleText:
                              '${context.l10n.delete}'
                              ' ${controller.plan!.planNumber}',
                          onAccept: () async {
                            final result = await controller.deletePlan();
                            if (context.mounted) {
                              if (result.isSuccess) {
                                await SuccessAlertWidget.showSuccessMessage(
                                  context,
                                  context.l10n.planDeletedSuccessfully,
                                );
                                if (context.mounted) {
                                  context.goNamed(RoutingPathName.planner);
                                }
                          ...
                          builder: (_) {
                            return Text(
                              context.l10n.areYouSureForDeletion,
              ...
              child: BuildColumnWithControl(
                ...
                control: EmrStepperFormField(
                  ...
                  validator: (value) => Validators.join([
                    ...
                    controller.schedulePlanController.loadsValidator(value),
                  ]),
                ),
              ),
            ),
