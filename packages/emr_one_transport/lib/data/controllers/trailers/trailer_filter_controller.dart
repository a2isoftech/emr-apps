...
class TrailerFilterController extends EmrFilterController {
  TrailerFilterController(this._sharedPrefs, {required this.dataController})
    : super(
        filterContentBuilder: (context, filter, controller) {
          switch (filter.key) {
            ...
            case StringConstants.trailerType:
              return EmrFilterAutoComplete<ListDetails, String>(
                Key(filter.key),
                filter,
                controller,
                (pattern, filterController) async {
                  final filtered = dataController.allTrailerTypes.where(
            ...
            case StringConstants.status:
              return EmrFilterAutoComplete<ListDetails, String>(
                ...
                controller,
                (pattern, filterController) async {
                  final filtered = dataController.allLifeCycleStatuses.where(
                    (e) => '${e.code} ${e.description}'.toLowerCase().contains(
                      pattern.toLowerCase(),
                    ),
                  );
  ...
  final TrailerDataController dataController;
  ...
  Future<void> bindSavedFiltersForUser() async {
    ...
    appliedFilters = await _sharedPrefs.getAssetsFilters(
      StringConstants.kTrailersFilterKey,
    );
  ...
  List<EmrFilterModel<dynamic>> _buildPreAppliedFilters(
    ...
    if (preAppliedFilters.assetTypes != null &&
      ...
      filterControls.add(
        EmrFilterModel<List<String>>(
          key: StringConstants.trailerType,
          name: StringConstants.trailerType,
  ...
  AssetsFilter getAppliedFilterValues() {
    ...
    final conditionsFilter = tryGetFilter<List<String>>(
      key: StringConstants.condition,
    );
    ...
    final statusesFilter = tryGetFilter<List<String>>(
      key: StringConstants.status,
    );
    ...
    final assetTypesFilter = tryGetFilter<List<String>>(
      key: StringConstants.trailerType,
    );
  ...
  List<EmrFilterModel<dynamic>> newFilters() {
    return [
      ...
      if (!containsFilter(key: StringConstants.trailerType))
        EmrFilterModel<List<String>>(
          key: StringConstants.trailerType,
