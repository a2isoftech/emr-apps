...
class ContainersDataSource extends EmrQueryDataSource<Containers>
    with EmrQueryDataSourcePager<Containers> {
  ContainersDataSource({required this.containerService});

  final ContainerService containerService;
  List<Containers> containersData = [];
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      final result = await containerService.searchContainers(payload);

      var containerSearchcount = 0;
      if (result.data != null && result.isSuccess) {
        containersData = result.data ?? [];
        containerSearchcount = result.totalItems;
      }

      recordCount = containerSearchcount;
      pageInfo.hasNextPage = pageNumber < (recordCount ~/ pageSize) + 1;
      pageInfo.hasPreviousPage = pageNumber > 1;
      data.value = containersData;
  ...
  void appendFiltersToSearch(PagedFilterRequest model) {
    ...
    if (queryScope.filters[StringConstants.containerTypeDisplay] != null) {
      final containertype = queryScope
          .filters[StringConstants.containerTypeDisplay] as List<String>;
      model.filters![StringConstants.containerTypeFilter] = containertype;
    }
    if (queryScope.filters[StringConstants.status] != null) {
      final statuses =
          queryScope.filters[StringConstants.status] as List<String>;
      model.filters![StringConstants.lifeCycleStatusFilter] = statuses;
    }
  ...
  String _getFacetTitle(String k) => switch (k) {
        ...
        StringConstants.typeFacet => StringConstants.containerTypeDisplay,
