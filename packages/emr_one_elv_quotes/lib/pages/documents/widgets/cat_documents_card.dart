...
class CatDocumentsCard extends StatefulWidget {
  const CatDocumentsCard({required this.vehicleCat, super.key});
  final VehicleCatDetailResponseModel vehicleCat;

  @override
  State<CatDocumentsCard> createState() => _CatDocumentsCardState();
}

class _CatDocumentsCardState extends State<CatDocumentsCard> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: controller.getImage(widget.vehicleCat.imagePath),
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          controller.removeImageFromCatList(widget.vehicleCat);
              ...
              await showDialog<void>(
                ...
                builder: (context) {
                  return ShowExpandedDialog(
                    title: context.l10n.catalyticConverter,
                    image: snapshot.data!,
                    bottomChildren: [
                      BottomTextWidget(
                        isInitialText: true,
                        ...
                        text: DateFormat('dd/MMM/yy hh:mm a')
                            .format(widget.vehicleCat.updatedAt),
                      ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DocumentInfoDetail(
                        ...
                        info: widget.vehicleCat.location,
                      ),
                      const SizedBox(
                        height: Insets.gutter / 4,
                      ),
                      DocumentInfoDetail(
                        ...
                        info: widget.vehicleCat.catClassification,
                      ),
                      const SizedBox(
                        height: Insets.gutter / 4,
                      ),
                      DocumentInfoDetail(
                        title: context.l10n.serialNo,
                        info: widget.vehicleCat.serialNumber ?? '',
                      ),
                      const SizedBox(
                        height: Insets.gutter / 4,
                      ),
                      DocumentInfoDetail(
                        ...
                        info: widget.vehicleCat.sealNumber ?? '',
                      ),
                      const SizedBox(
                        height: Insets.gutter / 4,
                      ),
                      DocumentInfoDetail(
                        ...
                        info: (widget.vehicleCat.catalyticFill ?? 0).toString(),
                      ),
                    ],
                  ),
                ),
